...
impl ConvLayer {
    fn validate(&self) -> Result<(), Box<dyn std::error::Error>> {
        if self.kernel_size != 3 {
            return Err("only 3x3 conv kernels are supported".into());
        }
        if self.weights.len()
    ...
    fn forward(
        ...
        let mut output = vec![vec![vec![0.0_f32; width]; height]; self.out_channels];
        for out_channel in 0..self.out_channels {
            for y in 0..height {
                for x in 0..width {
                    let mut acc = self.bias[out_channel];
                    for in_channel in 0..self.in_channels {
                        for ky in 0..self.kernel_size {
                            for kx in 0..self.kernel_size {
                                ...
                                let weight_index = (((out_channel * self.in_channels + in_channel)
                                    * self.kernel_size
                                    + ky)
                                    * self.kernel_size)
                                    + kx;
                                acc += input[in_channel][iy as usize][ix as usize]
                                    * self.weights[weight_index];
                            }
                        }
                    }
                    output[out_channel][y][x] = acc.max(0.0);
                }
            }
        }
        output
...
fn global_average_pool(input: &[Vec<Vec<f32>>], height: usize, width: usize) -> Vec<f32> {
    let norm = (height * width).max(1) as f32;
    input
        .iter()
        .map(|channel| {
            channel
